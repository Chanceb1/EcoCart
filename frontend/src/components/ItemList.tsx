...
export default function ItemList(): JSX.Element {
    ...
    // Add check for empty products array
        ...
        if (!hasSearchParams) {
            ...
            if (filterCategories.current.length === 0 && filterMethods.current.length === 0) {
                // no search string
                ...
                if (!searchString || searchString == '') {
                    return (
                        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {loadedItems.map((item) => {
                                if ((filterCategories.current && filterCategories.current.includes(item.category))
                                    || (filterMethods.current && filterMethods.current.includes(item.recycle_method))
                                ) {
                                    return <Item
                                        key={item.id}
                                        item={{
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {loadedItems.map(item => {
                                if (
                                    ...
                                    || (filterCategories.current && filterCategories.current.includes(item.category))
                                    || (filterMethods.current && filterMethods.current.includes(item.recycle_method))
                                ) {
                                    return <Item
                                        key={item.id}
